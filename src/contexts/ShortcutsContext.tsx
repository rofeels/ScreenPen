...
export function ShortcutsProvider({ children }: { children: ReactNode }) {
	...
	useEffect(() => {
		getIsMac()
			.then(setIsMac)
			.catch(() => {
				// Ignore platform detection failures and keep the default bindings visible.
			});
		...
		window.electronAPI
			...
			.catch(() => {
				// Ignore load failures and fall back to default shortcuts.
			});

		const unsubscribe = window.electronAPI.onShortcutsUpdated?.((nextShortcuts) => {
			if (nextShortcuts) {
				setShortcuts(mergeWithDefaults(nextShortcuts as Partial<ShortcutsConfig>));
			}
		});
