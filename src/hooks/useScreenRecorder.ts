...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const preparePermissions = useCallback(
		async (options: { startup?: boolean } = {}) => {
			...
			if (!screenPermission.success || screenPermission.status !== "granted") {
				...
				alert(
					options.startup
						? t("permissions.screenRecordingNeeded")
						: t("permissions.screenRecordingMissing"),
				);
				return false;
			}
			...
			alert(
				options.startup
					? t("permissions.accessibilityNeeded")
					: t("permissions.accessibilityMissing"),
			);
	...
	const cleanupCapturedMedia = useCallback(() => {
		...
		if (mixingContext.current) {
			mixingContext.current.close().catch(() => {
				// Ignore teardown failures while cleaning up the recorder graph.
			});
	...
	}, [selectMimeType, webcamDeviceId, webcamEnabled]);
	...
	const startRecording = async () => {
		...
		try {
			...
			if (!selectedSource) {
				alert(t("permissions.selectSource"));
				return;
			}
			...
			if (wantsAudioCapture) {
				...
				if (systemAudioEnabled) {
					try {
						...
						alert(t("permissions.systemAudioUnavailable"));
						screenMediaStream = await navigator.mediaDevices.getUserMedia({
							audio: false,
							video: browserScreenVideoConstraints,
						});
				...
				if (microphoneEnabled) {
					try {
						...
						alert(t("permissions.microphoneDenied"));
			...
			alert(
				error instanceof Error
					? t("permissions.failedToStart", undefined, { error: error.message })
					: t("permissions.failedToStartGeneric"),
			);
			setRecording(false);
			cleanupCapturedMedia();
