...
export function ExportDialog({
	...
	const formatLabel = exportFormat === "gif" ? "GIF" : "MP4";
	...
	const handleClickShowInFolder = async () => {
		if (exportedFilePath) {
			try {
				...
				if (!result.success) {
					const errorMessage = result.error || result.message || t("export.revealInFolderFailed");
				...
				toast.error(t("export.revealInFolderError", undefined, { error: errorMessage }));
			}
		}
	};
