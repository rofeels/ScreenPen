...
function Timeline({
	...
	return (
		...
		>
			...
			<Row id={ZOOM_ROW_ID} isEmpty={zoomItems.length === 0} hint={t("hints.addZoom")}>
				{zoomItems.map((item) => (
					<Item
						id={item.id}
						key={item.id}
			...
			<Row id={TRIM_ROW_ID} isEmpty={trimItems.length === 0} hint={t("hints.addTrim")}>
				{trimItems.map((item) => (
					<Item
						id={item.id}
						key={item.id}
			...
			<Row id={SPEED_ROW_ID} isEmpty={speedItems.length === 0} hint={t("hints.addSpeed")}>
				{speedItems.map((item) => (
					<Item
						id={item.id}
						key={item.id}
			...
			<Row id={AUDIO_ROW_ID} isEmpty={audioItems.length === 0} hint={t("hints.addAudio")}>
				{audioItems.map((item) => (
					<Item
						id={item.id}
						key={item.id}
...
export default function TimelineEditor({
	...
	const applyCustomAspectRatio = useCallback(() => {
		...
		if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
			toast.error(t("controls.customAspectPositive"));
			return;
		}
	...
	const handleAddZoom = useCallback(() => {
		...
		if (isOverlapping || gapToNext <= 0) {
			toast.error(t("zoom.cannotPlace"), {
				description: t("zoom.existsOrNoSpace"),
			});
			return;
		}
	...
	const handleSuggestZooms = useCallback(() => {
		...
		if (!onZoomSuggested) {
			toast.error(t("zoom.suggestHandlerUnavailable"));
			return;
		}

		if (cursorTelemetry.length < 2) {
			toast.info(t("zoom.noTelemetry"), {
				description: t("zoom.recordFirst"),
			});
			return;
		}
		...
		if (normalizedSamples.length < 2) {
			toast.info(t("zoom.noUsableTelemetry"), {
				description: t("zoom.notEnoughMovement"),
			});
			return;
		}
		...
		if (dwellCandidates.length === 0) {
			toast.info(t("zoom.noInteractionMoments"), {
				description: t("zoom.tryRecording"),
			});
			return;
		}
		...
		if (addedCount === 0) {
			toast.info(t("zoom.noAutoZoomSlots"), {
				description: t("zoom.dwellPointsOverlap"),
			});
			return;
		}

		toast.success(t("zoom.addedSuggestions", undefined, { count: addedCount }));
	}, [
		videoDuration,
		totalMs,
	...
	const handleAddTrim = useCallback(() => {
		...
		if (isOverlapping || gapToNext <= 0) {
			toast.error(t("trim.cannotPlace"), {
				description: t("trim.existsOrNoSpace"),
			});
			return;
		}
	...
	const handleAddSpeed = useCallback(() => {
		...
		if (isOverlapping || gapToNext <= 0) {
			toast.error(t("speed.cannotPlace"), {
				description: t("speed.existsOrNoSpace"),
			});
			return;
		}
	...
	const handleAddAudio = useCallback(async () => {
		...
		if (audioDurationMs <= 0) {
			toast.error(t("audio.couldNotReadFile"), {
				description: t("audio.unsupportedFile"),
			});
			return;
		}
		...
		if (isOverlapping || gapToNext <= 0) {
			toast.error(t("audio.cannotPlace"), {
				description: t("audio.existsOrNoSpace"),
			});
			return;
		}
		...
		onAudioAdded({ start: startPos, end: startPos + actualDuration }, audioPath);
	}, [videoDuration, totalMs, currentTimeMs, audioRegions, onAudioAdded, t]);
	...
	const timelineItems = useMemo<TimelineRenderItem[]>(() => {
		const zooms: TimelineRenderItem[] = zoomRegions.map((region, index) => ({
			...
			label: t("zoom.label", undefined, { index: index + 1 }),
		...
		const trims: TimelineRenderItem[] = trimRegions.map((region, index) => ({
			...
			rowId: TRIM_ROW_ID,
			span: { start: region.startMs, end: region.endMs },
			label: t("trim.label", undefined, { index: index + 1 }),
			variant: "trim",
		}));
		...
		const annotations: TimelineRenderItem[] = annotationRegions.map((region) => {
			...
			if (region.type === "text") {
				// Show text preview
				const preview = region.content.trim() || t("annotation.emptyText");
		...
		const speeds: TimelineRenderItem[] = speedRegions.map((region, index) => ({
			...
			rowId: SPEED_ROW_ID,
			span: { start: region.startMs, end: region.endMs },
			label: t("speed.regionLabel", undefined, { index: index + 1 }),
	...
	}, [zoomRegions, trimRegions, annotationRegions, speedRegions, audioRegions, t]);
		...
		return (
			<div className="flex-1 flex flex-col items-center justify-center rounded-lg bg-[#09090b] gap-3">
				...
				<div className="text-center">
					<p className="text-sm font-medium text-slate-300">{t("empty.title")}</p>
					<p className="text-xs text-slate-500 mt-1">{t("empty.description")}</p>
				</div>
			</div>
		);
	}
			...
			<div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-[#09090b]">
				<div className="flex items-center gap-1">
					...
					<Button
						...
						title={t("annotation.addAnnotation")}
					>
					...
					<DropdownMenu>
						...
						<DropdownMenuContent align="end" className="bg-[#1a1a1a] border-white/10">
							...
							<div className="px-2 py-1.5 flex items-center gap-2 text-slate-300">
								<span className="text-sm">{t("controls.custom")}</span>
								<input
									type="text"
									inputMode="numeric"
									...
									aria-label={t("controls.customWidth")}
								/>
								...
								<input
									...
									onKeyDown={handleCustomAspectRatioKeyDown}
									className="w-12 h-7 rounded border border-white/15 bg-black/20 px-1.5 text-sm text-slate-100 focus:outline-none focus:ring-1 focus:ring-[#2563EB]"
									aria-label={t("controls.customHeight")}
								/>
								<Button
									variant="ghost"
									size="sm"
				...
				<div className="flex items-center gap-4 text-[10px] text-slate-500 font-medium">
					<span className="flex items-center gap-1.5">
						<kbd className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[#2563EB] font-sans">
							{t("controls.sideScroll")}
						</kbd>
						<span>{t("controls.pan")}</span>
					</span>
					<span className="flex items-center gap-1.5">
						<kbd className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[#2563EB] font-sans">
							{scrollLabels.pan}
