...
export function ShortcutsConfigDialog() {
	...
	useEffect(() => {
		...
		const handleCapture = (e: KeyboardEvent) => {
			...
			if (found?.type === "fixed") {
				toast.error(
					t("shortcutsConfig.reserved", undefined, { label: tShortcuts(found.labelKey) }),
				);
				return;
			}
		...
		>
			<DialogContent className="bg-[#09090b] border-white/10 text-white max-w-[420px]">
									...
									<div className="flex items-center justify-between px-1 py-1.5 mb-0.5 bg-amber-500/10 border border-amber-500/20 rounded text-xs">
										<span className="text-amber-400">
											{t("shortcutsConfig.alreadyUsedBy", undefined, {
												action: tShortcuts(SHORTCUT_LABELS[conflict.conflictWith.action]),
											})}
										</span>
				...
				<div className="space-y-0.5 mt-2">
					...
					{FIXED_SHORTCUTS.map(({ labelKey, display }) => (
						<div
							key={labelKey}
							className="flex items-center justify-between py-1.5 px-1 border-b border-white/5 last:border-0"
						>
							<span className="text-sm text-slate-400">{tShortcuts(labelKey)}</span>
